...
// Runs a block on the main thread (CEF callbacks may arrive on the UI thread
// already, but dispatch_async keeps ordering simple and safe).
...
bool IsAllowedScheme(const std::string& scheme) {
    return scheme == "http" || scheme == "https" || scheme == ARCADIA_CACHE_SCHEME ||
           scheme == "about" /* about:blank for the start page */;
}

// Downloads the first favicon URL and forwards PNG bytes to the sink.
...
public:
    ...
    void OnDownloadImageFinished(const CefString&, int, CefRefPtr<CefImage> image) override {
        if (!image) { return; }
        int w = 0, h = 0;
        CefRefPtr<CefBinaryValue> data = image->GetAsPNG(1.0f, /*with_transparency*/ true, w, h);
        if (!data) { return; }
...
void ArcadiaClientHandler::OnLoadingStateChange(CefRefPtr<CefBrowser> browser,
    ...
    // When a live page finishes loading, run a tiny script-free probe for a
    // password field to offer login persistence (best-effort).
    if (!isLoading && offlineMode_ != CEFOfflineModeReplay) {
        // NOTE: we avoid executing site JS for this; instead use the render
        // process DOM visitor in a real build. Here we forward optimistically
        // when the page contains a password input (checked via DOM visitor in
        // CEFLoginDetection — wired in a follow-up). Left as a hook:
    }
}
...
bool ArcadiaClientHandler::OnBeforeBrowse(CefRefPtr<CefBrowser> browser,
    ...
    // Hard block every Chromium-internal / non-web scheme.
    if (!IsAllowedScheme(scheme)) {
        return true;  // cancel navigation
    }

    // Let the app veto too (e.g. policy). Block synchronously via the sink.
    ...
    return allowed ? false : true;  // false == allow
}
...
CefRefPtr<CefResourceRequestHandler> ArcadiaClientHandler::GetResourceRequestHandler(
    ...
    switch (offlineMode_) {
        ...
        case CEFOfflineModeReplay:
            disable_default_handling = true;  // serve only from snapshot
